...
function readProto(index, protoname, structs) {
  ...
  parser.on("opentag", function (tag) {
    ...
    switch (tag.name) {
      ...
      case "field":
        ...
        ["expression"].forEach(function (ff) {
          if (tag.attributes[ff]) {
            f[ff] = tag.attributes[ff];
          }
        });
      ...
      case "pad":
        parser.active.push({
          pad: parseInt(tag.attributes.bytes),
          type: "pad",
        });
        break;
      case "item":
  ...
  parser.on("closetag", function (tag) {
    ...
    switch (tag) {
      case "reply":
        ...
        break;
      case "item":
        parser.struct.body[parser.struct.body.length - 1].value =
          parser.lastValue;
        break;
      case "value":
      ...
      case "xcb":
        ...
        var reqsArr = []; //ugly
        for (s in structs) {
          if (structs[s].type == "request") reqsArr.push(structs[s]);
        }
  ...
  function fieldLength(f) {
    if (f.type === "pad") return f.pad;
    if (f.type === "valueparam") return 0; // depend on actual input, calculation inserted into generated code
    if (f.type === "list") return 0;
    if (f.type === "STRING8") return 0;
  ...
  function genReq(req, last) {
    ...
    if (req.body.length == 0) {
      ...
      for (var arg = 1; arg < req.body.length; ++arg) {
        ...
        if (field.type === "valueparam") {
          ...
          for (
            var i = arg - 1;
            i > 1;
            --i // we don't support param0 as a valuemask
          ) {
      ...
      if (extraLength)
        result.push(
          "    var data = Buffer.alloc(" + reqLen + " + extraLength);",
        );
      else {
        result.pop();
      ...
      if (req.body.length != 0) {
        ...
        for (var arg = 1; arg < req.body.length; ++arg) {
          ...
          if (f.type === "pad") {
            ...
            result.push(
              "    // value mask for " + valueListStruct.name + " parameters",
            );
            f.valueList.maskOffset = offset;
            offset += fieldLength(f);
          } else if (f.type === "valueparam") {
            if (f.mask) f.maskType = f.mask.type;
            else {
              f.maskOffset = offset;
      ...
      if (!extraLength)
        ...
        result.push(
          "    data.writeUInt16LE(" + reqLen4 + "+(extraLength+3)>>2, 2);",
        );
