...
function FrameBuffer(options) {
    ...
    // Called at the top of every submit(), which is the one call every
    // request ends in — the core proxy's and every hand-packed extension
    // request's alike. The client keeps its wire sequence numbers widenable
    // from here (XClient.prototype._requestSubmitted).
    this.onSubmit = null;
 ...
 *
 * The request must have taken its sequence number already (`seq_num++`
 * before submit, never after): `onSubmit` may queue a request of its own
 * behind it, which takes the next number.
 *
