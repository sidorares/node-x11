...
XClient.prototype.init = function(stream)
{
    ...
    const client = this;
    pack_stream.onSubmit = expectsReply => client._requestSubmitted(expectsReply);
...
// Wire sequence numbers are 16 bits, and _widenSeq can only rebuild the full
// value while fewer than 2^16 requests go out between two packets the server
// sends back — a successful void request sends nothing. So keep a
// reply-generating request in every 60000-request window, inserting a cheap
// GetInputFocus when a run of void requests reaches it (libxcb does the same).
//
// This runs from FrameBuffer.submit(), the one call every request ends in.
// It used to run in the core request proxy alone, which extension requests
// never pass through — they are packed by hand in lib/ext/*.js — so a frame of
// 65536 RENDER composites in a row wrapped the number with nothing answered in
// between, and the next reply was widened to the wrong request and dropped.
XClient.prototype._requestSubmitted = function(expectsReply)
{
    if (expectsReply) {
        this._last_seq_anchor = this.seq_num;
        return;
    }
    if (this._injecting_anchor || this.seq_num - this._last_seq_anchor < 60000)
        return;
    this._injecting_anchor = true;
    try {
...
// order, and _requestSubmitted guarantees an anchor (a reply) at least every
