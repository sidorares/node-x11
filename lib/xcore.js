    ...
    for (const r in reqs)
    {
        ...
        target[r] = (reqName => {
            ...
            if (templateType == 'function')
            {
                 ...
                 const reqPack = reqTemplate.apply(this, args);
                 ...
                 const format = reqTemplate[0];
                 const requestArguments = [];

                 for (const fixedArg of reqTemplate[1])
                     requestArguments.push(fixedArg);
                 for (const arg of args)
                     requestArguments.push(arg);
