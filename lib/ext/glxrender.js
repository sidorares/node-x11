...
module.exports = function (GLX, ctx) {
  ...
  function commandBuffer(opcode, len) {
    ...
    if (len > MAX_SMALL_RENDER)
      throw Error(
        "Buffer too big. Make sure you are using RenderLarge for large commands",
      );
  ...
  function serialize2ifv(opcode, i1, i2, fv) {
    ...
    res.writeUInt32LE(i1, 4);
    res.writeUInt32LE(i2, 8);
    for (var i = 0; i < fv.length; ++i) res.writeFloatLE(fv[i], 12 + i * 4);
    buffers.push(res);
  }
  ...
  function render(ctxLocal) {
    if (!ctxLocal)
      // ctxLocal overrides ctx passed during creation of renderContext
      ctxLocal = ctx;
  ...
  var renderContext = {
    ...
    Lightfv: function (light, name, p1, p2, p3, p4) {
      if (p1.length) serialize2i4f(87, light, name, p1[0], p1[1], p1[2], p1[3]);
      else serialize2i4f(87, light, name, p1, p2, p3, p4);
    },
    Materialfv: function (light, name, p1, p2, p3, p4) {
      if (p1.length) serialize2i4f(97, light, name, p1[0], p1[1], p1[2], p1[3]);
      else serialize2i4f(97, light, name, p1, p2, p3, p4);
    },
    Clear: function (mask) {
	...
	*/
      ...
      switch (type) {
        ...
        case constants.UNSIGNED_BYTE:
          for (var i = 0; i < data.length; ++i) res[60 + i] = data[i];
          break;
        default:
  ...
  for (var c in constants) renderContext[c] = constants[c];

  // bind some glx functions
  "NewList EndList GenLists GenTextures IsTexture SwapBuffers Finish"
    .split(" ")
