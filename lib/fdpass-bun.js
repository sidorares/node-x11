...
// errno is thread-local, so libc hands it out through a function — named
// differently by the two libcs. The reader thread dlopens it by this name too.
const ERRNO_SYMBOL_NAME = isDarwin ? '__error' : '__errno_location';
const ERRNO_SYMBOL = { [ERRNO_SYMBOL_NAME]: { args: [], returns: 'ptr' } };
...
function loadLibc() {
    ...
    for (const name of LIBC_CANDIDATES) {
        ...
        const errnoLocation = sym[ERRNO_SYMBOL_NAME];
        libc = {
            name,
            sym,
            ptr: ffi.ptr,
...
const lib = ffi.dlopen(workerData.libc, {
    ...
    close: { args: ['int'], returns: 'int' },
    [workerData.errnoSymbol]: { args: [], returns: 'ptr' }
});
const errnoLocation = lib.symbols[workerData.errnoSymbol];
const errno = () => ffi.read.i32(errnoLocation(), 0);
const POLLIN = 1, POLLOUT = 4, EINTR = 4;
...
while (running) {
    ...
    if (rc < 0) {
        // EINTR is not a failure: a signal landed on this thread while it
        // waited, and the wait simply resumes. It is also routine. The
        // runtime suspends every JavaScript thread with a signal (SIGPWR on
        // Linux) each time its WebAssembly compiler threads install newly
        // compiled code, and this thread is one of them — so a client that
        // loads a WebAssembly module sees dozens of these in a row while its
        // socket is quiet (a 32-function module tiering up produced 64
        // within 0.6 s, measured on Linux/arm64), and they must not count
        // towards giving up. A descriptor that has really gone bad shows up
        // as POLLNVAL below instead, so a poll that keeps failing for any
        // other reason is something this thread cannot fix — say so rather
        // than spin.
    ...
    _startReader() {
        ...
        try {
            ...
            this._worker = new Worker(POLLER_SOURCE, {
                eval: true,
                workerData: { libc: libc.name, errnoSymbol: ERRNO_SYMBOL_NAME, sock: dup, wake: pipefds[0] }
            });
        } catch (err) {
