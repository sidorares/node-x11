...
exports.requireExt = function (display, callback) {
  ...
  X.QueryExtension("GLX", function (err, ext) {
      ...
      X.replies[X.seq_num] = [
        function (buf, opt) {
          ...
          for (i = 0; i < numConfigs; ++i) {
            ...
            var names =
              "visualID visualType rgbMode redBits greenBits blueBits alphaBits accumRedBits accumGreen accumBlueBits accumAlphaBits doubleBufferMode stereoMode rgbBits depthBits stencilBits numAuxBuffers level".split(
                " ",
              );
    ...
    ext.CreateContext = function (ctx, visual, screen, shareListCtx, isDirect) {
      X.seq_num++;
      X.pack_stream.pack("CCSLLLLCxxx", [
        ext.majorOpcode,
        3,
        6,
        ctx,
    ...
    ext.Render = function (ctx, data) {
      X.seq_num++;
      var length = 0;
      if (Buffer.isBuffer(data)) length = 2 + data.length / 4;
      else if (Array.isArray(data)) {
        length = 2;
        for (var i = 0; i < data.length; ++i) length += data[i].length / 4;
      }
      X.pack_stream.pack("CCSL", [ext.majorOpcode, 1, length, ctx]);
      if (Buffer.isBuffer(data)) X.pack_stream.write_queue.push(data);
      else if (Array.isArray(data))
