...
exports.requireExt = function (display, callback) {
  ...
  X.QueryExtension("SHAPE", function (err, ext) {
    ...
    ext.Rectangles = function (
      ...
      if (ordering === undefined) ordering = ext.Ordering.Unsorted;

      var length = 4 + rectangles.length * 2;

      X.seq_num++;
      //            captureStack();
      X.pack_stream.pack("CCSCCCxLss", [
        ext.majorOpcode,
        1,
        length,
        op,
    ...
    ext.Mask = function (op, kind, window, x, y, bitmap) {
      X.seq_num++;
      //            captureStack();
      X.pack_stream.pack("CCSCCxxLssL", [
        ext.majorOpcode,
        2,
        5,
        op,
    ...
    X.eventParsers[ext.firstEvent + ext.events.ShapeNotify] = function (
      type,
      seq,
      extra,
      code,
