...
class XServer extends EventEmitter {
  constructor(servsock: net.Server, params: any) {
    super();
    const server = this;
    servsock.on("connection", function (stream: net.Socket) {
...
class XServerClientConnection extends EventEmitter {
  params: any;
  stream: net.Socket;
  ...
  constructor(stream: net.Socket, params: any) {
    super();
    this.params = params;
    const serv = this;
    serv.stream = stream;
    serv.pack_stream = new UnpackStream();
    serv.pack_stream.on("data", function (data: any) {
  ...
  readClientHandshake() {
    ...
    serv.pack_stream.unpackTo(
      ...
      function () {
        console.log(hello);
        serv.pack_stream.get(hello.authTypeLength, function (authType: Buffer) {
          serv.pack_stream.get(
            hello.authDataLength,
            function (authData: Buffer) {
  ...
  checkAuth(authType: string, authData: Buffer) {
    const serv = this;
  ...
  expectMessage() {
    ...
    serv.pack_stream.unpack("CCS", function (header: any) {
      serv.sequence++;
      console.log("Request:", header[0]);
      console.log("Extra:", header[1]);
      console.log("length:", header[2]);
      serv.pack_stream.get((header[2] - 1) * 4, function (reqBody: Buffer) {
...
export function createServer(
  options: any,
  onconnect: (client: XServerClientConnection) => void,
) {
