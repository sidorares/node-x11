...
x11.createClient(function (err, display) {
  ...
  X.GetKeyboardMapping(min, max - min, function (err, list) {
    for (var i = 0; i < list.length; ++i) {
      ...
      var sublist = list[i];
      for (var j = 0; j < sublist.length; ++j) name.push(chr2Data[sublist[j]]);
    }
  });

  // Launch a window to listen by key events:
  X.CreateWindow(wid, root, 0, 0, 100, 100, 0, 0, 0, 0, {
    backgroundPixel: white,
    eventMask: evKeyPress,
  });
  X.MapWindow(wid);

  X.on("event", function (ev) {
    if (ev.type == 2) {
      // filter by KeyPress. Useful if you have a more open eventMask.
      ...
      } else console.log(">> keyCode " + ev.keycode + " was not recognized.");
    }
  });
});
