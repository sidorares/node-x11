...
x11.createClient(function (err, display) {
  ...
  var wid = X.AllocID();
  X.CreateWindow(wid, display.screen[0].root, 100, 100, 900, 700, 0, 0, 0, 0, {
    backgroundPixel: display.screen[0].black_pixel,
  });
  X.MapWindow(wid);
  var wid1 = X.AllocID();
  X.CreateWindow(wid1, wid, 10, 10, 380, 680, 0, 0, 0, 0, {
  ...
  var surf = spawn("ssh", [
    ...
    "2222",
    "laplace@127.0.0.1",
    "surf -x -e " + wid1 + " http://192.168.43.149:4000",
  ]);
  surf.winbuf = "";
  surf.stdout.on("data", function handleWid(buf) {
  ...
  var wid2 = X.AllocID();
  X.CreateWindow(wid2, wid, 510, 10, 380, 680, 0, 0, 0, 0, {
    backgroundPixel: display.screen[0].white_pixel,
  });
  X.MapWindow(wid2);
