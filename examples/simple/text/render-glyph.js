...
const Exposure      = x11.eventMask.Exposure;
        ...
        const ft2 = require('freetype2_render');
        const fontface= ft2.parse(fs.readFileSync(process.argv[2]));
...
function padWidth(buf, width) {
  const height = buf.length / width;
  if (width %4 === 0)
    return buf;
  else {
    const stride = (width+3)&~3;
    const res = Buffer.alloc(height*stride);
    res.fill(0);
...
const xclient = x11.createClient({ debug: true }, (err, display) => {
    ...
    display.client.require('render', (err, Render) => {
       ...
       const size = parseInt(process.argv[3]*64);

       console.log(fontface.available_characters.length);
        const glyphs = fontface.available_characters.map(ch => fontface.render(ch, size, 0, 96, 0));
        const glyphFromCode = [];
        glyphs.forEach(g => {
        ...
        while (pos + 1 < text.length) {
          const kern = fontface.kerning(text.charCodeAt(pos), text.charCodeAt(pos+1), size, 0, 96, 0);
        ...
        function draw(x, y) {
          ...
          const yoff = 2*parseInt(process.argv[3]);
