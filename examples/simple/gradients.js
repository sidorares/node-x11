...
x11
  .createClient(function (err, display) {
    ...
    X.require("render", function (err, Render) {
      ...
      Render.CreatePicture(picture, win, Render.rgb24, {
        polyEdge: 1,
        polyMode: 0,
      });
      ...
      Render.CreatePicture(pix_pict, pixmap, Render.rgba32, {
        polyEdge: 1,
        polyMode: 0,
      });
      ...
      Render.CreatePicture(pix_pict1, pixmap1, Render.rgba32, {
        polyEdge: 1,
        polyMode: 0,
      });
      Render.Composite(
      ...
      Render.CreatePicture(pix_pict2, pixmap2, Render.rgba32, {
        polyEdge: 1,
        polyMode: 0,
      });
      for (var i = 0; i < 100; ++i) {
        var pts = [];
        for (var coord = 0; coord < 6; coord++) pts.push(Math.random() * 500);
        Render.Triangles(
          3,
          pic_grad,
          Math.random() * 2500,
