...
x11.createClient(function (err, display) {
  ...
  X.require("glx", function (err, GLX) {
    ...
    for (visual in visuals) {
      if (visuals[visual].class == 4 || visuals[visual].class == 5) break;
    }

    var win = X.AllocID();
    X.CreateWindow(win, root, 0, 0, width, height, 0, 0, 0, visual, {
      eventMask: eventmask,
    });
    X.MapWindow(win);
    var ctx = X.AllocID();
    ...
    GLX.GenLists(ctx, 1, function (err, startListIndex) {
      ...
      for (var i = 0; i < 1000; ++i) {
        ...
        gl.Color3f(
          randomarr[i + 1000][0] + 15 / 30,
          (randomarr[i + 1000][1] + 15) / 30,
          (randomarr[i + 1000][2] + 15) / 30,
        );
      }
      gl.End();
      gl.render(ctx);
