...
var xclient = x11.createClient(function (err, display) {
  ...
  X.require("glx", function (err, GLX) {
    ...
    X.CreateWindow(win, root, 0, 0, width, height, 0, 0, 0, 0, {
      eventMask: x11.eventMask.PointerMotion,
    });
    X.MapWindow(win);
    GLX.MakeCurrent(win, ctx, 0, function (err, res) {
      ...
      for (var i = 0; i < 10000; ++i) {
        ...
        gl.Color3f(
          randomarr[i + 10000][0] + 15 / 30,
          (randomarr[i + 10000][1] + 15) / 30,
          (randomarr[i + 10000][2] + 15) / 30,
        );
      }
      gl.End();
      gl.render(ctx);
      ...
      X.on("event", function (ev) {
        ...
        if (list) {
          ...
          for (var i = 0; i < 500; ++i) {
            gl.Vertex3f(randomarr[i][0], randomarr[i][1], randomarr[i][2]);
            gl.Color3f(
              randomarr[i + 500][0] + 15 / 30,
              (randomarr[i + 500][1] + 15) / 30,
              (randomarr[i + 500][2] + 15) / 30,
            );
          }
        }
