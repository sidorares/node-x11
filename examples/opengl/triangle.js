// Minimal indirect-GLX example: draw a colored triangle in a window.
// The X server must allow indirect GLX contexts (start it with +iglx),
// see docs/ext/glx.md.
...
x11.createClient((err, display) => {
    ...
    X.require('glx', (err, GLX) => {
        ...
        GLX.GetVisualConfigs(0, (err, configs) => {
            ...
            const win = X.AllocID();
            X.CreateWindow(win, root, 0, 0, width, height, 0, depth, 1, cfg.visualID,
            ...
            X.on('event', ev => {
                if (ev.name !== 'MapNotify' || started)
                    return;
                started = true;
                // small delay: XQuartz needs the drawable realized
                setTimeout(draw, 100);
            });
