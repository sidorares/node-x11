...
var xclient = x11.createClient(function (err, display) {
  ...
  X.require("render", function (err, rendExt) {
    ...
    X.CreateWindow(wid, root, 10, 10, 400, 300, 0, 0, 0, 0, {
    ...
    X.on("event", function (ev) {
      if (ev.type == 4 && ev.keycode == 1) pressed = true;
      else if (ev.type == 5 && ev.keycode == 1) pressed = false;
      else if (ev.type == 5 && ev.keycode == 4) {
        gradNo--;
        if (gradNo < 0) gradNo = 0;
        console.log(gradNo);
      } else if (ev.type == 5 && ev.keycode == 5) {
        gradNo++;
        if (gradNo > 9) gradNo = 9;
        console.log(gradNo);
      } else if (pressed) draw(ev.x, ev.y);
