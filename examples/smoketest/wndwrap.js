...
function GraphicContext(win) {
  ...
  this.xclient.CreateGC(this.id, win.id, {
...
GraphicContext.prototype.polyLine = function (points, opts) {
  ...
  if (opts && opts.coordinateMode === "previous") coordinateMode = 1;
...
GraphicContext.prototype.copy = function (
  srcDrawable,
  srcX,
  srcY,
  dstX,
...
function Window(parent, x, y, w, h, bg) {
  ...
  this.__defineSetter__("title", function (title) {
    this._title = title;
    this.xclient.ChangeProperty(
      0,
      this.id,
      this.xclient.atoms.WM_NAME,
      this.xclient.atoms.STRING,
      8,
      title,
    );
  });
...
Window.prototype.createPixmap = function (width, height) {
  ...
  this.xclient.CreatePixmap(
    this.xclient.display.screen[0].root_depth,
    pid,
    this.id,
    width,
    height,
