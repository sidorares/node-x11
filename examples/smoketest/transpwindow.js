...
x11
  .createClient(function (err, display) {
    ...
    X.CreateWindow(wid, root, 10, 10, 168, 195, 1, depth, 1, visual, {
    ...
    X.require("render", function (err, Render) {
      ...
      function randomLinear() {
        var stops = [];
        for (var i = 0; i < 3; ++i) stops.push(Math.random());
        stops.sort();
        var colors = [];
        ...
        Render.LinearGradient(
          ...
          [0, 0],
          [
            100 + parseInt(Math.random() * 500),
            parseInt(100 + Math.random() * 300),
          ],
          colors,
        );
        return gradient;
      ...
      for (var i = 0; i < 50; ++i) gradients.push(randomLinear());
