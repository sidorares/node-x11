...
function Hexy(buffer, config) {
  ...
  this.toString = () => {
    ...
    for (let i = 0; i!= line_arr.length; ++i) {
      const hex_raw = line_arr[i], hex = hex_raw[0], raw = hex_raw[1];
      //insert spaces every `self.format.twos` or fours
      let howMany = hex.length;
      ...
      let hex_formatted = "";
      const middle = Math.floor(self.width / 2)-1;
      const groupSpaces = (new Array(self.groupSpacing+1)).join(' ');
      for (let j=0; j<hex.length; j+=howMany) {
        const s = hex.substr(j, howMany);
  ...
  const lines = () => {
    ...
    for (let i = 0; i<self.buffer.length ; i+=self.width) {
      const begin = i, end   = i+self.width >= buffer.length ? buffer.length : i+self.width, slice = buffer.slice(begin, end), hex   = self.caps === "upper" ? hexu(slice) : hexl(slice), raw   = slice.toString('ascii');
  ...
  const hexu = buffer => hexl(buffer).toUpperCase()
