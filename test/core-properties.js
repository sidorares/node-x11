    ...
    X.GetProperty(
      ...
      function (err, prop) {
        ...
        assert.equal(
          propvalset,
          propvalget,
          "get property result different from set property value",
        );
        done();
      },
    );
    ...
    X.on("event", function (ev) {
      if (ev.name === "PropertyNotify") {
        assert.equal(
          ev.atom,
          X.atoms.WM_NAME,
          "atom in notification should be same as in ChangeProperty",
        );
        ...
        assert.equal(
          ev.wid,
          wid,
          "window in notification should be same as in ChangeProperty",
        );
        done();
        return;
      }
        ...
        X.GetProperty(
          ...
          function (err, prop) {
            assert.equal(
              prop.type,
              0,
              "non-existent property type should be 0",
            );
            assert.equal(
              prop.data.length,
              0,
              "non-existent property data length should be 0",
            );
            done();
          },
        );
